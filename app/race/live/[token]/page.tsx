      ...
      {isRacing && !finished && (
        <div className={`absolute top-24 left-3 z-[1000] rounded-xl shadow-2xl ${
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
            <div>
              <div className="text-sm uppercase tracking-wide text-slate-500">BTM</div>
              <div className="text-2xl font-bold font-mono text-slate-900 tabular-nums leading-tight">
                {btmDeg != null ? `${Math.round(btmDeg)}°` : '—'}
              </div>
            </div>
            <div>
              <div className="text-sm uppercase tracking-wide text-slate-500">Speed</div>
              <div className="text-2xl font-bold font-mono text-slate-900 tabular-nums leading-tight">
                {currentPos ? currentPos.speed_kts.toFixed(1) : '—'}<span className="text-[10px] text-slate-500 ml-0.5">kts</span>
              </div>
            </div>
            <div>
              <div className="text-sm uppercase tracking-wide text-slate-500">ETA</div>
              <div className="text-sm font-semibold font-mono text-slate-900 tabular-nums leading-tight">
                {timeToMarkLabel}
              </div>
            </div>
            <div>
              <div className="text-sm uppercase tracking-wide text-slate-500">Dist</div>
